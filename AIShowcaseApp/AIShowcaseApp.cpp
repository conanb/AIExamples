...
bool AIShowcaseApp::startup() {
	...
	for (auto& knight : m_knights) {
		...
		auto myFunc = new std::function<void(GameObject*)>();

		*myFunc = [this](GameObject* gameObject) {
			gameObject->getBlackboard().set("health", 100.f);
			auto n = m_pathNodes[rand() % m_pathNodes.size()];
			gameObject->setPosition(n->x, n->y);

			std::list<Pathfinding::Node*>* path = nullptr;
			if (gameObject->getBlackboard().get("path", &path))
				path->clear();
			Vector2* velocity = nullptr;
			if (gameObject->getBlackboard().get("velocity", &velocity)) {
	...
	for (auto& caveman : m_cavemen) {
		...
		caveman.getBlackboard().set("respawnFunction", new std::function<void(GameObject*)>(
			...
			do {
				...
				gameObject->setPosition(8 + 16.0f * (i % m_map->getWidth()), 8 + 16.0f * (i / m_map->getWidth()));
			} while (m_tiles[i] != -1);
			Vector2* velocity = nullptr;
			if (gameObject->getBlackboard().get("velocity", &velocity)) {
				velocity->x = velocity->y = 0;
...
Force WaterAvoidanceForce::getForce(GameObject* gameObject) const {
...
Force SeekClosestForce::getForce(GameObject* gameObject) const {

	float xDiff = 0;
	float yDiff = 0;

	GameObject* target = nullptr;
	if (gameObject->getBlackboard().get("target", &target) &&
		target != nullptr) {
