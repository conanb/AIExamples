...
eBehaviourResult NavMesh::FollowPathBehaviour::execute(GameObject* gameObject, float deltaTime) {

	// access data from the game object
	std::list<Vector2>* smoothPath = nullptr;
	if (gameObject->getBlackboard().get("smoothpath", &smoothPath) == false ||
		smoothPath->empty())
...
eBehaviourResult NavMesh::NewPathBehaviour::execute(GameObject* gameObject, float deltaTime) {
